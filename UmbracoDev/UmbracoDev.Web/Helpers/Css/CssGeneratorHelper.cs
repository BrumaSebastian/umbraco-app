        ...
        foreach (var property in elementProperties)
        {
            ...
            switch (value)
            {
                case MediaWithCrops<Image> image when image is not null:
                    ...
                    break;
                case int intValue when property.Name.Contains(PERCENT, StringComparison.InvariantCultureIgnoreCase) && intValue > 0:
                    string propertyName = property.Name.TrimEnd(PERCENT);
                    cssBuilder.Append($"{ConvertToCssProperty(elementPrefix, propertyName)}:{intValue}%; ");
                    break;
